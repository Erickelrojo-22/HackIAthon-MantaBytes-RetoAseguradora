...
from fraudia_claims.models import compute_anomaly_points
...
def _model_alerts(anomaly: pd.DataFrame, nlp: pd.DataFrame, offset: int = 0) -> pd.DataFrame:
    rows: list[dict] = []
        ...
        rows.append(
            {
                ...
                "codigo": "NLP-01",
                "categoria": "Narrativa",
                "severidad": "Alta" if row.score_nlp == 20 else "Media",
...
def score_claims(tables: dict[str, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    scores[["score_reglas", "score_anomalia", "score_nlp"]] = scores[["score_reglas", "score_anomalia", "score_nlp"]].fillna(0).astype(int)
    scores["regla_critica"] = scores["regla_critica"].fillna(False).astype(bool)
    scores["score_final"] = (scores["score_reglas"].clip(upper=60) + scores["score_anomalia"] + scores["score_nlp"]).clip(upper=100)
    ...
    generated_alerts = _model_alerts(anomaly, nlp, offset=offset)
    ...
    return scores.sort_values("score_final", ascending=False).reset_index(drop=True), alerts.reset_index(drop=True)
